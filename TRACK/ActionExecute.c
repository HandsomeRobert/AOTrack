		...
		if(ActionExecuteQueue != NULL)
		{
			...
			switch(actionType)
			{
				case Message_TrrigerCamera:									//Trriger Camera
						...
						{
							case DigitalOutput_1: MCUIO = GPIO_PIN_7;break;
							case DigitalOutput_2: MCUIO = GPIO_PIN_8;break;
							case DigitalOutput_3: MCUIO = GPIO_PIN_9;break;
							case DigitalOutput_4: MCUIO = GPIO_PIN_11;break;						
							case DigitalOutput_5: MCUIO = GPIO_PIN_12;break;
							case DigitalOutput_6: MCUIO = GPIO_PIN_13;break;
							case DigitalOutput_7: MCUIO = GPIO_PIN_14;break;
							case DigitalOutput_8: MCUIO = GPIO_PIN_15;break;
							default:break;
						}				
						HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_SET);			//拉高电平
						AddDelayTimeToList(MCUIO, GPIO_PIN_RESET, systemTickCount + pActionExcuteTempTriggerCamera->OutputDuration);
//						vTaskDelay(pActionExcuteTempTriggerCamera->OutputDuration);
//						HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_RESET);		//拉低电平
				...
				case Message_TrrigerOutput:									//Trriger Output
					pActionExcuteTempSetOutput = (propActionSetOutput*)actionInfoTemp->pAction;
						...
						{
							HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_RESET);			//拉低电平
							AddDelayTimeToList(MCUIO, GPIO_PIN_SET, systemTickCount + pActionExcuteTempSetOutput->OutputDuration);
...
//							HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_SET);				//拉高电平
						...
						{
							HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_SET);			//拉高电平
							AddDelayTimeToList(MCUIO, GPIO_PIN_RESET, systemTickCount + pActionExcuteTempSetOutput->OutputDuration);
//							vTaskDelay(pActionExcuteTempSetOutput->OutputDuration);
//							HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_RESET);		//拉低电平
						}
					break;
				...
				case Message_TrrigerPush:										//Trriger Push
					pActionExcuteTempPushOut = (propActionPushOut*)actionInfoTemp->pAction;
						...
						HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_SET);			//拉高电平
						AddDelayTimeToList(MCUIO, GPIO_PIN_RESET, systemTickCount + pActionExcuteTempPushOut->PushLength);
...
//						HAL_GPIO_WritePin(GPIOH,MCUIO,GPIO_PIN_RESET);		//拉低电平
		...
		{
			if(delayActionBuffer[i].isActionUsed)
			{
				if(systemTickCount > delayActionBuffer[i].delayOverTime)
				{
