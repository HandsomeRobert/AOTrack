...
void lwip_comm_default_ip_set(__lwip_dev *lwipx)
{
	...
	//默认远端IP为:192.168.66.10，，则检测程序Inspecion为192.168.66.11/12/13...
	...
	//默认本地IP为:192.168.66.2 ==>与PLC的IP一样
	lwipx->ip[0]=192;	
	lwipx->ip[1]=168;
	lwipx->ip[2]=66;
	lwipx->ip[3]=2;
...
#endif
	Netif_Init_Flag=netif_add(&lwip_netif,&ipaddr,&netmask,&gw,NULL,&ethernetif_init,&tcpip_input);//提供网卡接口（驱动），向网卡列表中添加一个网卡驱动
																																						//当有多个网卡时，可创建多个lwip_netif结构体(netfi.c/h中)，构成一个数据链
