...
UBaseType_t x;
	...
	by the port. */
	#if( portSTACK_GROWTH < 0 )          //HCDS计算任务堆栈顶部地址
	{
	...
	for( x = ( UBaseType_t ) 0; x < ( UBaseType_t ) configMAX_TASK_NAME_LEN; x++ )  //HCDS存储任务名，任务名大于规定长度，后面自动忽略
	{
	...
	if( uxPriority >= ( UBaseType_t ) configMAX_PRIORITIES )  //HCDS保证优先级必须小于最大优先级
	{
	...
	#if ( configUSE_MUTEXES == 1 )        //HCDS使能互斥信号量
	{
	...
	vListInitialiseItem( &( pxNewTCB->xStateListItem ) ); //HCDS初始化任务控制块中的列表项
	vListInitialiseItem( &( pxNewTCB->xEventListItem ) );

	/* Set the pxNewTCB as a link back from the ListItem_t.  This is so we can get
	back to	the containing TCB from a generic item in a list. */
	listSET_LIST_ITEM_OWNER( &( pxNewTCB->xStateListItem ), pxNewTCB );    //HCDS设置列表项的所有者为当前任务控制块

	/* Event lists are always in priority order. */
	listSET_LIST_ITEM_VALUE( &( pxNewTCB->xEventListItem ), ( TickType_t ) configMAX_PRIORITIES - ( TickType_t ) uxPriority ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
	listSET_LIST_ITEM_OWNER( &( pxNewTCB->xEventListItem ), pxNewTCB );

	#if ( portCRITICAL_NESTING_IN_TCB == 1 )      //HCDS使能临界区嵌套
	{
		pxNewTCB->uxCriticalNesting = ( UBaseType_t ) 0U;
	}
	#endif /* portCRITICAL_NESTING_IN_TCB */

	#if ( configUSE_APPLICATION_TASK_TAG == 1 )     //HCDS使能任务标签功能
	{
		pxNewTCB->pxTaskTag = NULL;
	}
	#endif /* configUSE_APPLICATION_TASK_TAG */

	#if ( configGENERATE_RUN_TIME_STATS == 1 )       //HCDS使能时间统计功能
	{
	...
	#if ( configUSE_TASK_NOTIFICATIONS == 1 )  //任务消息通知机制
	{
	...
	#if( INCLUDE_xTaskAbortDelay == 1 )  //HCDS使能函数xTaskAbortDelay()
	{
